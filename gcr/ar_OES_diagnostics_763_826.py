...
def plot_ALEXIS_data():
    os.chdir('C:/Users/ivan/OneDrive/Research/ar_OES/DATA/763_826/meta_scale_images')
...
def plot_allports():
    ...
    plt.legend(loc = 2)
    plt.savefig('vport_ALL.png', dpi=300)
    plt.show(fig3)
    print(slope, intercept)
plot_allports()

# %%
# copy the following line into alltemps function to model meta frac as function of line ratio
# mscale = 3.58775946328 * line_ratio - 12.338615998 ; print(mscale)
...
temp_array, temp_int, mfit = get_all_temps_763_826()
print(temp_array.T)
...
def meta_comp_826_852():
    os.chdir('C:/Users/ivan/OneDrive/Research/ar_OES/DATA/763_852')
    from scipy import stats
    ...
    plt.legend(loc = 2)
    plt.savefig('m_compare.png', dpi=300)
    plt.show(fig1)
meta_comp_826_852()
...
def exp_theor_temp_comp():
    ...
    slope, intercept, r_value, p_value, std_err = stats.linregress(model_dat[:,0], exp_temp_data[:,0])
...
def exp_temp_ratio():
    ...
    os.chdir("C:/Users/ivan/OneDrive/Research/ar_OES/DATA/763_826")
    exp_temp_data = np.loadtxt("ALLPORTS.dat", delimiter = "|")
    slope, intercept, r_value, p_value, std_err = stats.linregress(exp_temp_data[:,2], exp_temp_data[:,0])
...
def mfit_dens():
    ...
    from scipy import stats
    import matplotlib.pyplot as plt
    os.chdir("C:/Users/ivanS/OneDrive/Research/ar_OES/DATA/763_826")
    exp_temp_data = np.loadtxt("ALLPORTS.dat", delimiter = "|")
    slope, intercept, r_value, p_value, std_err = stats.linregress(exp_temp_data[:,1], exp_temp_data[:,3])
    r_squared = r_value ** 2
